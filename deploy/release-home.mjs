// ============================================================================
// deploy/release-home.mjs — point the live Home (landing) at one immutable commit.
// ----------------------------------------------------------------------------
// Usage:
//   node deploy/release-home.mjs <full-40-char-commit-hash>
//
// Webflow is NEVER touched by this. Releasing (and rolling back) only updates
// the Bunny manifest that the stable Home boot loader (boot/home.js) reads:
//
//   release  -> node deploy/release-home.mjs <new-commit>
//   rollback -> node deploy/release-home.mjs <previous-commit>
//
// Mirrors deploy/release-library.mjs exactly, for Home assets. Credentials are
// read ONLY from .env.local and are NEVER written to the repo, printed, or
// logged. Only the PRESENCE of each var is reported.
// ============================================================================
...
const MANIFEST_PUBLIC_URL = `${CDN_BASE}/boot/home-release.json`;
const BOOT_PUBLIC_URL = `${CDN_BASE}/boot/home.js`;
const BOOT_REMOTE_PATH = "boot/home.js"; // path inside the Bunny storage zone
const MANIFEST_REMOTE_PATH = "boot/home-release.json";

const BOOT_SRC = resolve(ROOT, "boot/home.js");
const MANIFEST_SRC = resolve(ROOT, "boot/home-release.json");
...
function jsdelivrLoaderUrl(hash) {
  return `https://cdn.jsdelivr.net/gh/${REPO}@${hash}/webflow/home-loader.js`;
}
...
if (!hash) {
  die(
    "missing commit hash.\n" +
    "  Usage: node deploy/release-home.mjs <full-40-char-commit-hash>\n" +
...
if (missing.length) {
  ...
  log(
    ...
    "    node deploy/release-home.mjs " + hash + "\n"
  );
  process.exit(0);
}
...
// and rarely changes); warn loudly if Bunny still serves different bytes.
{
  ...
  if (liveBoot !== bootBytes.toString("utf8")) {
    log(
      "  ! boot/home.js changed and the cached copy still differs.\n" +
...
log("\n✓ Home release complete — commit " + hash + " is now live via the manifest.");
