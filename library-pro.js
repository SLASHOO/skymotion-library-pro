(() => {
  ...
  async function handleFilterOptionClick(btn, label, s) {
    ...
    history.push({
      stepIndex,
      prevChatHTML: isProMobilePortrait() ? "" : chat.innerHTML,
      prevState: { ...state }
    });
...
function setProTab(tab) {
  ...
  function getProPackItems() {
    ...
    return [
      ...
      {
        id: "travel_creator",
        title: "Travel Creator Pack",
        creator: "SkyMotion",
        label: "PACK",
        thumb: travelCover,
        meta: `${Math.max(12, moves.length || 0)} moves · ${Math.max(3, plans.length || 0)} plans · Pro tips`,
        description: "A complete pack for cinematic travel content on location."
      },
      {
        id: "urban_creator",
        title: "Urban Creator Pack",
        creator: "SkyMotion",
        label: "COMING SOON",
        thumb: pickThumb(moves.find((m) => hasMatch(m?.env, "urban"))?.thumb, moves?.[0]?.thumb, travelCover),
        meta: "City moves · Short plans · Safety tips",
        description: "For streets, buildings, tight spaces and city-style reveals."
      },
      {
        id: "beginner_safe",
        title: "Beginner Safe Pack",
        creator: "SkyMotion",
        label: "COMING SOON",
        thumb: pickThumb(moves.find((m) => difficultyTone(getMoveDifficulty(m)) === "basic")?.thumb, moves?.[0]?.thumb, travelCover),
        meta: "Safe moves · Simple plans · Practice flow",
        description: "For cinematic results without risky moves or complex flying."
      }
    ];
  }
  ...
  function renderProMobileHome() {
    ...
    const popularMoves = moves.slice(0, 4);
    const cinematicPlans = plans.slice(0, 2);
