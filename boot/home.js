/* ============================================================================
 * boot/home.js — SkyMotion Home (landing) STABLE boot loader (Bunny-hosted)
 * ----------------------------------------------------------------------------
 * This is the ONE script Webflow's Home page references, forever:
 *
 ...
 *        https://skymotion-cdn.b-cdn.net/boot/home-release.json
 *      { "release": "<40-char git commit>",
 *        "loader":  "https://cdn.jsdelivr.net/gh/SLASHOO/skymotion-library-pro@<commit>/webflow/home-loader.js" }
 *   2. Strictly validate the manifest (repo + path + full hash + consistency).
 *   3. Inject that immutable jsDelivr Home loader into <head>.
 *
 * Releasing a new Home version = updating the Bunny manifest only. Webflow is
 * never touched again. GitHub stays the source of truth; the pinned jsDelivr
 * loader derives every other asset URL from its own @commit src.
 *
 * This file MUST NOT change Home UI, the jsDelivr home-loader itself, or the
 * Library boot system. It only orchestrates manifest -> loader.
 * ============================================================================ */
(function () {
  "use strict";

  // --- Single-init guard (Webflow page transitions / accidental double embed).
  if (window.__SM_HOME_BOOT__) return;
  window.__SM_HOME_BOOT__ = true;

  var MOUNT_ID = "sm-home";

  // The manifest URL is fixed to Bunny. window.SM_HOME_BOOT_MANIFEST_URL exists
  // ONLY so local smoke tests can point at a local fixture (mirrors the existing
  // window.SM_ASSET_BASE convention in home-loader.js). Production never sets it.
  var MANIFEST_URL =
    window.SM_HOME_BOOT_MANIFEST_URL ||
    "https://skymotion-cdn.b-cdn.net/boot/home-release.json";

  // Exactly one repo + one loader path are ever allowed.
  var LOADER_RE =
    /^https:\/\/cdn\.jsdelivr\.net\/gh\/SLASHOO\/skymotion-library-pro@([0-9a-f]{40})\/webflow\/home-loader\.js$/;
  ...
  function showError(reason) {
    ...
    console.error("[sm-home-boot] " + reason);
    ...
    function paint() {
      ...
      // Self-contained fallback: inline styles only, no dependency on Home CSS.
      ...
      msg.textContent = "Home couldn't load. Please refresh.";
  ...
  function validateManifest(m) {
    ...
    if (!match) {
      ...
      return "manifest.loader must be the jsDelivr SLASHOO/skymotion-library-pro " +
             "/webflow/home-loader.js URL at a full commit";
    }
    if (match[1] !== release) {
